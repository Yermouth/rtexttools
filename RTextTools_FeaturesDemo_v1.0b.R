...
data_subset <- data[sample(1:30000,size=1000,replace=FALSE),]

# Create a term frequency matrix for the training/classification process.
data_matrix <- create_matrix(data_subset$TITLE, language="spanish", removeSparseTerms=0.9998, removeNumbers=TRUE, minWordLength=4)

# Create a container that combines your term frequency matrix and the corresponding training codes.
data_container <- create_container(data_matrix,data_subset$CODIGO,trainSize=1:250, testSize=251:500)
...
cvmaxent <- cross_validate(fnct=train_maxent,data_container,nfold=3,type="MAXENT",seed=7310)
cvmaxent
...
analytics <- create_analytics(data_container,cbind(SVM_CLASSIFY,NAIVE_CLASSIFY,BOOSTING_CLASSIFY,BAGGING_CLASSIFY, RF_CLASSIFY,GLMNET_CLASSIFY,TREE_CLASSIFY,NNET_CLASSIFY,MAXENT_CLASSIFY))
